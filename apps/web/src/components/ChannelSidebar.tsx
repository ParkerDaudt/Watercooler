...
import { Hash, Lock, User, Plus, Megaphone, ChevronDown, ChevronRight, Settings, Volume2, MicOff, HeadphoneOff, Camera, MonitorUp } from "lucide-react";
        ...
        <div className="ml-6 space-y-0.5 mt-0.5">
          {voiceParticipants.map((p) => (
            <div key={p.userId} className="flex items-center gap-1.5 px-2 py-0.5 text-xs text-[var(--muted-foreground)]">
              ...
              {p.isScreenSharing && <MonitorUp size={10} className="text-green-400 shrink-0" />}
              {p.isVideoOn && !p.isScreenSharing && <Camera size={10} className="text-green-400 shrink-0" />}
